...
def test_load_bundle_reads_sanitized_representative_cli_bundle_fixture():
    bundle = HandoffLoader().load_bundle(REPRESENTATIVE_FIXTURE_BUNDLE_DIR)

    assert bundle.chronicle_records is not None
    assert len(bundle.chronicle_records) == 3
    assert bundle.manifest_payload["referenced_record_count"] == 4
    assert bundle.manifest_payload["eligible_context_count"] == 2
    assert bundle.handoff_payload["query"] == "release planning follow-up context"
    assert bundle.handoff_payload["import_validation"]["graph_export_node_count"] == 8
    assert bundle.graph_payload["export_manifest"]["tool_version"] == "sanitized-fixture"
...
def test_load_bundle_rejects_missing_required_files(tmp_path):
    ...
    assert excinfo.value.error_code == "bundle_validation.missing_required_file"
    assert excinfo.value.error_category == "bundle_validation"
...
def test_load_bundle_rejects_missing_file_listed_in_manifest(tmp_path):
    bundle_dir = tmp_path / "bundle"
    bundle_dir.mkdir()
    (bundle_dir / "graph.json").write_text(
        json.dumps(
    ...
    assert excinfo.value.error_code == "bundle_validation.missing_required_file"
    assert "EXTRA_NOT_PRESENT.json" in str(excinfo.value)


def test_load_bundle_rejects_contract_consistency_mismatch(tmp_path):
    bundle_dir = tmp_path / "bundle"
    bundle_dir.mkdir()
    (bundle_dir / "graph.json").write_text(
        json.dumps(
    ...
    assert excinfo.value.error_code == "bundle_validation.contract_consistency"
    assert "bundle_manifest.json handoff_contract_version" in str(excinfo.value)
