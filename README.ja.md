...
- `bash scripts/bootstrap_plugin_env.sh`
- `source .env.local.plugins`
- `OPENAI_COMPATIBLE_HOSTED_ENABLED=true` に変更
- `OPENAI_COMPATIBLE_HOSTED_MODEL` を設定
- `OPENAI_COMPATIBLE_HOSTED_API_KEY` を設定
- 標準の OpenAI hosted path を使うなら `OPENAI_COMPATIBLE_HOSTED_BASE_URL=https://api.openai.com` のまま、別の OpenAI-compatible provider を使うなら base URL を差し替える
...
mapping を含めて non-interactive に起動できます。
...
ローカル `act` で release verify を再現する場合:
...
notes job を確認し、publish は tag push または GitHub 側の workflow 実行に
委ねます。
